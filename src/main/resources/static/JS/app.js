(function (window, document) {
  ...
  const ROLE_ACCESS = {
    administrador: ['menu.html', 'cadprodutos.html', 'cadfornecedores.html', 'movimentacoes.html', 'relatorios.html', 'gestaodeusuario.html'],
    gestor: ['menu.html', 'movimentacoes.html', 'relatorios.html'],
    'operador de estoque': ['menu.html', 'cadprodutos.html', 'cadfornecedores.html'],
  };

  const ROLE_LANDING = {
    administrador: 'Menu.html',
    gestor: 'Menu.html',
    'operador de estoque': 'Menu.html',
  };
  ...
  function getSessionUserId() {
    const session = app.getSession();
    const parsed = Number(session && session.userId);
  ...
  function getAuthHeaders() {
    ...
    return {
      Authorization: `${tokenType} ${session.token}`,
    };
  }
  ...
  function makeFetch(url, options = {}) {
    ...
    return window.fetch(url, fetchOptions).then((response) => {
      if (response.status === 401) {
        app.clearSession();
        setTransientNotice('Sessão expirada. Faça login novamente.', 'error');
